...
class GPConvergenceMonitor:
    ...
    def __init__(
        ...
        *,
        ...
        self.prequential: list[dict[str, float]] = []
        self.stable_checks = 0
        self.best_actual = -math.inf
        self.no_best_improvement_steps = 0

    def observe_bo_result(self, actual: float, prediction: dict[str, Any]) -> None:
        actual = float(actual)
        mean = float(prediction["gp_pred_mean"])
        self.prequential.append({"actual": actual, "predicted": mean})
    ...
    def add_check(
        ...
        *,
        ...
        window = self.prequential[-self.prequential_window :]
        preq = prediction_metrics(
            [row["actual"] for row in window],
            [row["predicted"] for row in window],
        )
        holdout = holdout_metrics or {}
        current_mae = holdout.get("mae")
        current_spearman = holdout.get("spearman")
        current_preq = preq.get("mae")
        has_holdout_error = current_mae is not None
        previous = self.history[-1] if self.history else None
        relative_mae = self._relative_decrease(None if previous is None else previous.get("holdout_mae"), current_mae)
        ...
        if previous is not None and current_spearman is not None and previous.get("holdout_spearman") is not None:
            spearman_improvement = float(current_spearman) - float(previous["holdout_spearman"])
        preq_change = None
        ...
        row = {
            ...
            "holdout_spearman": current_spearman,
            "holdout_coverage_95": holdout.get("predictive_95_coverage"),
            "prequential_window_mae": current_preq,
            "prequential_window_rmse": preq.get("rmse"),
    ...
    def stop_decision(self, online_samples: int, elapsed_seconds: float, estimated_next_seconds: float) -> ConvergenceDecision:
        ...
        if int(online_samples) >= self.min_bo_samples and self.no_best_improvement_steps >= self.best_acc_patience:
            return ConvergenceDecision(False, "bo_best_acc_stagnated")
        if int(online_samples) >= self.max_bo_samples:
            return ConvergenceDecision(False, "sample_budget_reached")
        if int(online_samples) >= self.min_bo_samples and float(elapsed_seconds) + float(estimated_next_seconds) > self.max_wall_seconds:
            return ConvergenceDecision(False, "time_budget_reached")
        return ConvergenceDecision(False, None)
